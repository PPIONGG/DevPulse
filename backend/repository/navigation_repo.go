...
func (r *NavigationRepo) ListAll(ctx context.Context) ([]models.NavigationItem, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT id, label, icon, path, is_hidden, min_role, sort_order, created_at, updated_at
	...
	for rows.Next() {
		...
		err := rows.Scan(&i.ID, &i.Label, &i.Icon, &i.Path, &i.IsHidden, &i.MinRole, &i.SortOrder, &i.CreatedAt, &i.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func (r *NavigationRepo) ListVisible(ctx context.Context, userRole string) ([]models.NavigationItem, error) {
	// Simple role logic: admin sees everything not explicitly hidden for admins (which is none yet)
	// Users see items where min_role is 'user' AND is_hidden is false
	query := `SELECT id, label, icon, path, is_hidden, min_role, sort_order, created_at, updated_at
	          FROM navigation_items 
